...
const NFS_SOURCES = ['NFS Server 1', 'NFS Server 2', 'NFS Server 3'];
const DELIMITERS = [
  { label: 'Comma (,)', value: ',' },
  { label: 'Pipe (|)', value: '|' },
  { label: 'Tab', value: '\t' },
...
const FileSourceConfig: React.FC<FileSourceConfigProps> = ({ data, onChange }) => {
  ...
  const handleDataTypeChange = (header: string, type: string) => {
    const updatedTypes = { ...dataTypes, [header]: type };
    setDataTypes(updatedTypes);
    updateParentData({ dataTypes: updatedTypes });
  };
    ...
    <Box>
      ...
      {(fileSource === 'SFTP' || fileSource === 'AWS S3' || fileSource === 'NFS') && (
        <Box sx={{ mb: 2 }}>
          ...
          >
            ...
            {(fileSource === 'SFTP' ? SFTP_SOURCES : fileSource === 'AWS S3' ? AWS_SOURCES : NFS_SOURCES).map((source) => (
      ...
      {previewData.length > 0 && (
        <Box sx={{ mb: 2 }}>
          ...
          >
            <Table stickyHeader size="small" sx={{ minWidth: headers.length * 120 }}>
              <TableHead>
                <TableRow>
                  {headers.map((header) => (
      ...
      {headers.length > 0 && (
        <Box sx={{ mb: 2 }}>
          <FilterBuilder headers={headers} onFilterChange={(query) => console.log('Filter Query:', query)} />
        </Box>
      )}
